...
Four real gaps remain, stated plainly rather than papered over: no ambiguous issue, no spam example, no null-label example, no high-priority example. The question/discussion-only gap flagged in the last version of this matrix has since closed organically, through real operation, not by curating an example to fill it in - consistent with this project's own philosophy of preferring real operation over synthetic curation, which also rules out fabricating the four gaps that remain.
